...
const summaryRules = [
  ...
  {
    title: '特殊结算',
    text: '斗地主、掼蛋、炸金花、红中麻将按各自奖池或牌局分数单独结算。'
  }
]
...
const gameRules = [
  ...
  },
  {
    ...
    tag: '二到六人',
    entryFee: 10,
    play: '2 到 6 人参与。每名玩家选择 1 到 6 的一个点数，系统摇出一颗骰子作为开奖结果。',
    win: '猜中开奖结果的玩家获胜；可能多人同时猜中，也可能无人猜中。',
    scoring: [
      '猜中的玩家各 +10 分，猜错的玩家各 -10 分。',
      '如果所有玩家都猜中，则所有人都 +10 分；如果无人猜中，则本局平局不扣分。'
    ]
  },
  {
    key: 'blackjack',
  ...
  },
  {
    ...
    tag: '经典棋盘',
    entryFee: 30,
    play: '两名玩家轮流落子，棋盘为 14 × 14。先手执黑，后手执白；每手限时 30 秒。',
    win: '任意横、竖、斜方向先连成 5 子的一方获胜；30 秒内未落子自动判负；棋盘下满仍无人连五则为平局。',
    scoring: [
      '胜者 +30 分，败者 -30 分。',
      '平局不增减积分。'
    ]
  },
  {
    ...
    tag: '残局博弈',
    entryFee: 50,
    play: '两名玩家按中国象棋规则轮流走子，红方先行。系统会校验合法走法和将帅照面；每手限时 60 秒。',
  ...
  },
  {
    ...
    win: '共打 3 局。每局按出完顺序结算升级：同队包揽前两名升 3 级，头游和三游同队升 2 级，其他情况升 1 级。3 局结束后级牌更大的队伍获胜。',
    scoring: [
      '胜方每人获得：100 分 + 级牌差距 × 10 分；负方每人扣除同等分数。',
      '级牌差距按 3 局结束后的双方级牌计算。',
      '例如 3 局后胜方为 7、负方为 4，级差为 3，本场每名胜方玩家 +130 分，负方玩家 -130 分。'
    ]
  },
  {
    key: 'zha_jin_hua',
    ...
    tag: '筹码奖池',
    entryFee: 10,
    play: '2 到 5 人每人 3 张牌。玩家可看牌、跟注、加注、比牌或弃牌；看牌后按明牌金额下注。',
    win: '其他玩家弃牌或比牌出局后，最后留在场上的玩家获胜；也可摊牌比较牌型，牌型大者获胜。',
    scoring: [
      '门票 10 分；暗牌跟牌 5 分，明牌跟牌 10 分。',
      '加注按当前下注翻倍；暗牌单次下注上限 20 分，明牌单次下注上限 40 分。',
      '跟牌最多持续 5 轮，5 轮后场上未弃牌的玩家自动开牌。',
